...
describe("projectDetector module", () => {
  describe("detectProjectType", () => {
    ...
    test("should detect Node.js project with package.json", () => {
      // Spy on fs functions
      readdirSpy = spyOn(fs, "readdirSync").mockReturnValue(["package.json", "index.js", "node_modules"] as any);
      existsSpy = spyOn(fs, "existsSync").mockImplementation((filePath: PathLike) => {
        const pathStr = String(filePath);
        return pathStr.includes("package.json");
      });
      readFileSpy = spyOn(fs, "readFileSync").mockReturnValue('{"dependencies": {}, "devDependencies": {}}');
    ...
    test("should detect Ruby project", () => {
      ...
      readdirSpy = spyOn(fs, "readdirSync").mockReturnValue(["Gemfile", "Gemfile.lock", ".ruby-version"] as any);
      existsSpy = spyOn(fs, "existsSync").mockImplementation((filePath: PathLike) => {
        const pathStr = String(filePath);
    ...
    test("should detect Python project", () => {
      // Spy on fs functions
      readdirSpy = spyOn(fs, "readdirSync").mockReturnValue(["requirements.txt", "main.py", "setup.py"] as any);
      existsSpy = spyOn(fs, "existsSync").mockImplementation((filePath: PathLike) => {
        const pathStr = String(filePath);
    ...
    test("should handle directory without project files", () => {
      ...
      readdirSpy = spyOn(fs, "readdirSync").mockReturnValue([] as any);
    ...
    test("should detect React project from package.json", () => {
      // Spy on fs functions
      readdirSpy = spyOn(fs, "readdirSync").mockReturnValue(["package.json", "src"] as any);
      existsSpy = spyOn(fs, "existsSync").mockImplementation((filePath: PathLike) => {
        const pathStr = String(filePath);
        return pathStr.includes("package.json");
      });
      readFileSpy = spyOn(fs, "readFileSync").mockReturnValue('{"dependencies": {"react": "^18.0.0", "react-dom": "^18.0.0"}, "devDependencies": {}}');
  ...
  describe("getProjectFiles", () => {
    ...
    test("should list project files", () => {
      // Spy on fs functions
      readdirSpy = spyOn(fs, "readdirSync").mockReturnValue(["file1.js", "file2.py"] as any);
